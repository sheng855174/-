...
public class FireBall extends Fly implements Runnable
{
    ...
    public FireBall(int x,int y,Controller controller,String direction)
    {
        super(x+10,y,controller);
        width = 300;
        height = 200;
        String path = "img/MagicianPeople/";
        fire_right = new ImgSequence();
        fire_left = new ImgSequence();
        try{
            fire_right.addImge(ImageIO.read(new File(path+"fire_right.png")));
            fire_left.addImge(ImageIO.read(new File(path+"fire_left.png")));
        }
        catch (Exception ex) {
